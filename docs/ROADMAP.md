...
> 当前唯一有效的排期以本节为准。下面的 R2—R4 内容保留为历史实现台账，不代表当前应继续
> 按原顺序扩张。正式赛题基线见 `CONTEST_BASELINE.md`。

## 当前状态真值（2026-07-22）

| 项目 | 真实状态 |
| --- | --- |
| 正式赛题 | XH-202620 面向一流学科建设的学科垂类大模型与创新应用开发 |
| 正式链路 | Vue/FastAPI Demo → 星辰 Agent → 星火/科技文献模型、MaaS → 自研工具；规则上允许其他工具，但这是项目已冻结的参赛策略 |
| 论文语料 | 5 篇登记，1 篇 `development_seed`，0 篇双审/冻结 Gold |
| 外部模型 | 默认离线规则；已有星辰客户端、4个工具接口和OpenAPI，但未在平台真实注册/调用 |
| 检索 | Demo/hash 后端，不是比赛级真实 RAG |
| 多论文/机会 | 框架存在，真实数据不足时返回 `insufficient_evidence` |
| 用户验证 | 尚无至少 2 名真实目标用户的可提交记录 |
| 构建 | 后端157通过/0跳过；前端生产构建通过；离线Playwright黄金流程1项通过；真实基础设施Playwright本轮未执行 |

## P0：比赛基础功能闭环

- [x] 以正式 PDF 纠正合规基线：平台是推荐资源而非官方唯一准入方式。
- [x] 根据发榜方、官网展示和已审核v2.0方案，冻结讯飞能力为项目正式比赛主链路。
- [x] 恢复 `AGENTS.md`、方案设计、交接说明和 Markdown 跟踪规则。
- [ ] P0-X1：发布星辰最小真实工作流，打通追问、单篇拆解工具、星火组织、证据门禁和Vue运行态。
  - [x] 实现4个星辰工具接口、统一响应契约、OpenAPI、Bearer门禁和契约测试。
  - [x] 前端分离“本地演示就绪”和“讯飞正式链路就绪”，并收敛为三个核心工作台。
  - [x] 定稿星辰系统提示词、P0-X1工作流节点和发布门禁。
  - [ ] 将工具网关部署为星辰可访问的HTTPS服务，并在平台导入OpenAPI。
  - [ ] 发布星辰工作流，配置星火/科技文献模型并完成首次真实trace。
  - [ ] 把星辰返回的工作流/模型/工具版本和证据状态显示在Vue会话中。
- [ ] P0-X2：完成3个典型问题的星辰真实trace、P50/P95、错误率和失败案例。
- [ ] P0-M1：定稿MaaS“TAD实验协议风险诊断”数据契约与盲测集，完成ServiceID、训练曲线和基础/微调对比。
- [ ] 冻结 3 个典型问题、人工标准答案、EvidenceAnchor 和评分规则。
- [ ] 制作 5—10 篇合法 TAD Gold；先冻结其中 3 篇支撑典型问题与多论文闭环，并区分元数据、解析和语义审核状态。
- [ ] 把检索、单篇拆解、实验/图表解释、多篇比较和 Claim 诊断收敛为一条学生可独立完成的主线。
- [ ] 将关键词分流升级为真实多轮澄清和歧义消解；保留可测试的离线降级。
- [ ] 对基础功能 1—4 分别提供代码、测试、截图/视频和正确性证据。

## P0：效果验证与提交证据

- [ ] 先进行 5 人形成性测试并修正首次使用问题。
- [ ] 收集至少 2 名真实目标用户的结构化试用记录。
- [ ] 量化任务完成时间、准确率、证据定位成功率和人工判断一致性。
- [ ] 冻结 3 分钟真实交互视频脚本和黄金演示数据。
- [ ] 建立 `01`—`07` 提交物追踪表，按 2026-09-05 倒排。
- [ ] 视组织能力完成 50 人以上测试，作为额外加分证据，不伪造缺失数据。

## P1：技术与创新扩展

- [ ] 重点验证复杂问题求解、内容多元和可复制推广；个性化在有真实用户状态后再做。
- [ ] 用对比实验说明 RAG、关系证据和多模态解析分别带来的收益。

## 暂停项

在 P0 完成前，不新增 PDF 解析器、数据库迁移、复杂图谱页面、通用权限、计费或与 3 个典型问题
无关的后台模块。已有实现保留并修复，不继续横向铺开。
...
- [x] 完成汇报前核心演示链路可用性加固（当时将星辰接入后推；该排期已被顶部P0-X1取代）。
...
- [x] 建立可解释的演示启动与首次使用引导闭环。
  ...
  - [x] 前端顶部区分“本地演示/本地证据基础设施”和“讯飞正式链路”，提供五步演示向导，依次导航核心链、实验意图、真实图表、EvidenceAnchor用途和Claim论证路径。
  - [x] 单元、Vue生产构建、离线Playwright和真实基础设施Playwright均覆盖就绪状态与向导；当时星辰只是可选语言层，该产品决策已被顶部v2.0主链路取代。
