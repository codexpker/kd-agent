...
type DocumentStructure = {
  ...
  sections: { id: string; title: string; level: number; page_start: number | null; page_end: number | null }[]
  artifacts: { id: string; artifact_type: 'figure' | 'table'; label: string; caption: string | null; page: number | null }[]
  references: { id: string; artifact_id: string; text: string; page: number | null }[]
  warnings: string[]
}
type GraphNode = {
  node_id: string
...
const graphLayout = computed(() => {
  ...
  const visible = graph.value.nodes.filter((node) => node.node_type !== 'paper' && node.node_type !== 'narrative_move')
  const columns = [
    visible.filter((node) => node.node_type === 'claim'),
    visible.filter((node) => node.node_type === 'experiment' || node.node_type === 'artifact'),
    visible.filter((node) => node.node_type === 'evidence'),
  ]
  const height = Math.max(460, ...columns.map((items) => items.length * 58 + 70))
  ...
  return {
    height,
    nodes: [...placed.values()],
    edges: graph.value.edges.flatMap((edge) => {
...
function focusClaim(id: string) {
  focusedClaimId.value = focusedClaimId.value === id ? null : id
}
  ...
  <main class="paper-reader" data-testid="paper-reader">
    ...
    <template v-else-if="paper && structure">
      <section class="reader-integrity" data-testid="reader-integrity">
        ...
        <p v-else>当前结构来自 {{ structure.parser_name }} {{ structure.parser_version }}；PDF 原文件仍不写数据库、不进入 Git。</p>
      </section>
      ...
      <div class="reader-grid">
        <aside class="document-pane">
          ...
          <div class="pdf-placeholder" data-testid="pdf-permission-state">
            <div class="paper-sheet"><span>PDF</span><i></i><i></i><i></i><b>原文未在网页分发</b></div>
            <p v-if="structure.source === 'gold_snapshot'">缺少已持久化的授权解析结果，仅展示不会伪造位置的结构快照。</p>
            <p v-else>已加载客观版面结构；原文件访问仍受权利依据控制。</p>
          </div>
          ...
          <nav class="section-outline" aria-label="论文章节">
            <p>章节索引</p>
            <button v-for="(section, index) in structure.sections" :key="section.id" @click="activeTab = 'story'">
        ...
        <section class="analysis-pane">
          ...
          <section v-if="activeTab === 'story'" class="story-view" data-testid="narrative-chain">
            <header><small>PROBLEM → GAP → METHOD → EVIDENCE → BOUNDARY</small><h2>作者如何建立论证</h2><p>以下是审核数据中的语义拆解，不等同于 PDF 客观版面事实。</p></header>
            <ol>
        ...
        <aside class="evidence-pane" data-testid="evidence-inspector">
          ...
          <template v-if="selectedEvidence">
            ...
            <dl><div><dt>核验状态</dt><dd>{{ selectedEvidence.verified ? '已人工核验' : '待授权全文与人工复核' }}</dd></div><div><dt>原文位置</dt><dd>{{ selectedEvidence.page ? `第 ${selectedEvidence.page} 页` : '不可定位：页码尚未核验' }}</dd></div><div><dt>判断类型</dt><dd>development seed annotation</dd></div></dl>
            <button class="locate-button" disabled>{{ selectedEvidence.page ? `页码已记录：第 ${selectedEvidence.page} 页（原文未分发）` : 'PDF 定位已硬阻断' }}</button>
          </template>
          <div class="anchor-catalog"><p>全部 EvidenceAnchor</p><button v-for="item in paper.evidence" :key="item.id" :class="{ active: selectedEvidenceId === item.id }" @click="inspectEvidence(item.id)"><span>{{ item.id }}</span><div><b>{{ item.label }}</b><small>{{ item.verified ? '已核验' : '待核验' }} · {{ item.page ? `p.${item.page}` : 'no page' }}</small></div></button></div>
...
.card-view > article { display: grid; grid-template-columns: 88px 1fr; gap: 16px; margin-bottom: 11px; padding: 16px; border: 1px solid #dce3de; border-radius: 12px; background: white; transition: opacity .2s; }.card-view > article.muted { opacity: .3; }.card-index { display: grid; place-items: center; height: 64px; border-radius: 10px; background: #dfeee6; color: #215b43; font-weight: 900; }.card-body h3 { margin: 2px 0 12px; font-family: Georgia, serif; font-size: 18px; }.card-body dl { margin: 0; }.card-body dl > div { display: grid; grid-template-columns: 110px 1fr; padding: 9px 0; border-top: 1px solid #edf0ed; }.card-body dt { color: #7a8780; font-size: 9px; font-weight: 800; }.card-body dd { margin: 0; color: #35493f; font-size: 10px; line-height: 1.55; }.card-body dd > span { display: inline-block; margin: 0 4px 4px 0; padding: 4px 6px; border-radius: 5px; background: #f1f4f2; }.artifact-mark { height: 80px; display: grid; align-content: center; padding: 10px; border-radius: 9px; background: #e1eee8; color: #235e46; }.artifact-mark.table { background: #fff0dc; color: #8a581a; }.artifact-mark small { font-size: 8px; text-transform: uppercase; }.artifact-mark b { margin-top: 4px; font-size: 13px; }
.reader-graph { padding: 17px; overflow: hidden; border: 1px solid #dce3de; border-radius: 12px; background: white; }.reader-graph > header { display: flex; justify-content: space-between; }.reader-graph > header > span { color: #47725e; font-size: 9px; }.graph-labels { display: grid; grid-template-columns: repeat(3, 1fr); margin: 18px 0 3px; color: #819087; font-size: 8px; font-weight: 900; text-align: center; }.reader-graph svg { width: 100%; max-height: 720px; border-radius: 10px; background: #f5f7f5; }.reader-graph line { stroke: #c6d1ca; stroke-width: 1; }.reader-graph g { cursor: pointer; }.reader-graph rect { fill: white; stroke: #a8b9af; }.reader-graph g.experiment rect { fill: #e2eee8; stroke: #74a087; }.reader-graph g.artifact rect { fill: #fff0dc; stroke: #d39b56; }.reader-graph g.evidence rect { fill: #f1efff; stroke: #9d94c7; }.reader-graph text { fill: #31463b; font-size: 8px; }.reader-graph > p { color: #7e8a83; font-size: 8px; }
.evidence-kind { display: flex; justify-content: space-between; margin-top: 20px; color: #ef683e; font-size: 9px; font-weight: 900; text-transform: uppercase; }.evidence-pane h3 { margin: 12px 0; font-family: Georgia, serif; font-size: 21px; }.evidence-pane blockquote { margin: 0; padding: 14px; border-left: 3px solid #ef683e; background: #fff7ed; color: #4d4a3d; font-family: Georgia, serif; font-size: 12px; line-height: 1.65; }.evidence-pane dl { margin: 14px 0; }.evidence-pane dl > div { display: grid; grid-template-columns: 72px 1fr; padding: 8px 0; border-bottom: 1px solid #e5eae6; }.evidence-pane dt { color: #859188; font-size: 8px; }.evidence-pane dd { margin: 0; color: #3f5348; font-size: 9px; }.locate-button { width: 100%; padding: 10px; border: 0; border-radius: 8px; background: #205c45; color: white; font-size: 9px; font-weight: 900; }.locate-button:disabled { background: #dfe4e0; color: #89938d; }
