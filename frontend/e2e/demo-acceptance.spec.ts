...
test('synthetic_smoke_test: offline golden demo remains evidence-bounded and executable', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('evidence-inspector')).toContainText('不可定位：页码尚未核验')
  await expect(page.getByRole('button', { name: 'PDF 定位已硬阻断' })).toBeDisabled()
  ...
  await expect(page.getByTestId('paper-evidence-graph').locator('svg g')).toHaveCount(21)
