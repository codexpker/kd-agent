...
test('synthetic_smoke_test: offline golden demo remains evidence-bounded and executable', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('assistant-storage')).toContainText('进程内临时会话')
  ...
  await expect(page.getByTestId('assistant-graph-path')).toHaveCount(5)

  await page.goto('/papers/anomaly-transformer-2022')
  await expect(page.getByTestId('paper-reader')).toBeVisible()
  await expect(page.getByTestId('reader-integrity')).toContainText('无授权解析结果')
  ...
  await expect(page.getByTestId('evidence-usage')).toContainText('它在论证链中做什么')
